...
function renderPackages() {
  ...
  visible.forEach(pkg => {
    ...
    button.addEventListener('click', () => startPackage(pkg));
...
function renderResults() {
  ...
  list.innerHTML = `<ul class="results-list">${results.map(result => {
    ...
    return `<li class="result-item"><strong>${result.packageTitle}</strong><br>Score: ${result.score}/${result.total}<br><span>${date}</span></li>`;
  }).join('')}</ul>`;
}

function wireEvents() {
  state.dictation = new Dictation({ onFinish: renderFinish });
